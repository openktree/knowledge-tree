...
async def _prepare_create(
    ...
    result = await HatchetPipeline(state, user_id=input.user_id).create(
...
async def _prepare_rebuild(
    ...
    pipeline = HatchetPipeline(state, user_id=input.user_id)
...
async def generate_definition(input: NodePipelineInput, ctx: Context) -> dict:
    ...
    result = await HatchetPipeline(state, user_id=input.user_id).definition(node_id_str)
...
async def edge_task(input: UpdateEdgesInput, ctx: Context) -> dict:
    ...
    result = await HatchetPipeline(state, user_id=input.user_id).edges(

"""Wave planner prompt and helpers — canonical version sourced from workers/orchestrator.py.

Used by:
- workers/orchestrator.py           (OrchestratorWorker._plan_wave)
- hatchet/workflows/exploration.py  (_plan_wave helper)

agents/wave_planner.py has its own copy and is intentionally left separate.
"""
...
def parse_scope_plans(raw: str, wave_explore: int, wave_nav: int) -> list[ScopePlan]:
    ...
    Raises WavePlanParseError if the response cannot be parsed, allowing
    the caller to retry with error feedback to the LLM.
    """

...
export default function FactDetailPage({
  ...
  const fetchFact = useCallback(async (factId: string) => {
    setIsLoading(true);
    setError(null);
    try {
      const [factResult, nodesResult] = await Promise.all([
        api.facts.get(factId),
        api.facts.getNodes(factId),
      ]);
      setFact(factResult);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 pt-6 pb-4">
        <h1 className="text-2xl font-bold tracking-tight">Fact Detail</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Detailed view of a single fact with sources
        </p>
      </div>
            ...
            {fact.metadata && Object.keys(fact.metadata).length > 0 && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <pre className="text-xs bg-muted/50 rounded-md p-4 overflow-auto">

"""Composite Perspective Agent — LangGraph sub-agent that documents a
...
class PerspectiveAgentState(BaseModel):
    """LangGraph state for the composite perspective agent."""
...
def _build_perspective_graph(ctx: AgentContext) -> StateGraph:
    """Build the LangGraph StateGraph for composite perspective documentation.
    ...
    async def agent_node(state: PerspectiveAgentState) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("Error in composite perspective agent LLM call")
            return {
                "definition": "Perspective documentation failed: the LLM call encountered an error. Check logs for details.",
                "phase": "done",
            }
        return {"messages": [response]}

    async def tool_node(state: PerspectiveAgentState) -> dict[str, Any]:
    ...
    def should_continue(state: PerspectiveAgentState) -> str:
    ...
    def after_tools(state: PerspectiveAgentState) -> str:
        """Route after tool_node."""
        if state.phase == "done":
            return END
        return "agent"

    graph = StateGraph(PerspectiveAgentState)
...
async def build_perspective_impl(
    ctx: AgentContext,
    source_node_ids: list[str],
    claim: str,
    parent_concept: str = "",
) -> dict[str, Any]:
    """Generate a perspective node definition by documenting a claim from
    multiple source nodes.

    Uses a LangGraph sub-agent that reads source nodes via tools and produces
    a comprehensive, evidence-driven perspective definition.

    Args:
        ctx: Agent context with graph engine, model gateway, etc.
        source_node_ids: UUIDs of the source nodes to draw evidence from.
        claim: The perspective/claim/position to document.
        parent_concept: Optional parent concept this perspective belongs to.

    Returns:
        Dict with ``"definition"`` (str) and ``"facts_referenced"`` (list[str]).
    """
    if not source_node_ids:
        logger.warning("build_perspective_impl called with no source_node_ids")
        return {
            "definition": "No source nodes provided for perspective documentation.",
            "facts_referenced": [],
        }

    await ctx.emit(
        "activity_log",
        action=f"Documenting perspective '{claim}' from {len(source_node_ids)} source nodes",
        tool="composite_perspective",
    )
    ...
    for nid in source_node_ids:
        ...
        if node is not None:
            ...
            logger.warning("Composite perspective: source node %s not found", nid)

    parent_section = ""
    if parent_concept:
        parent_section = f"\n## Parent Concept\nThis perspective belongs to the broader concept: **{parent_concept}**\n"
    ...
    initial_state = PerspectiveAgentState(
        ...
        messages=[
            ...
            HumanMessage(
                content=(
                    "Read ALL source nodes and their facts, then call "
                    ...
                    "Analyze every argument's strengths and weaknesses. "
                    "Consider motivations of advocates and opponents. "
                    "Report where the overall evidence landscape points — "
                    "are most nodes/facts supportive, challenging, or neutral?"
                )
            ),
        ],
    )
    ...
    try:
        graph = _build_perspective_graph(ctx)
        compiled = graph.compile()

        recursion_limit = max(len(source_node_ids) * 6 + 10, 50)
        ...
        if not definition:
            logger.info("Composite perspective agent ended without calling finish — extracting from messages")
            msgs = final.get("messages", []) if isinstance(final, dict) else final.messages
            for msg in reversed(msgs):
                if isinstance(msg, AIMessage):
                    text = _extract_text_content(msg.content)
        ...
        if not definition:
            definition = "Perspective documentation completed but no definition was produced."

...
async def test_resolve_edges_skips_recent_pairs() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "other_node", [fact_id])])
...
async def test_resolve_edges_creates_edge_from_llm() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "quantum_mechanics", [fact_id])])
...
async def test_resolve_edges_skips_none_decisions() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "unrelated_b", [fact_id])])
    ctx.graph_engine.get_recent_edge_pairs = AsyncMock(return_value=set())
    ctx.graph_engine.get_evaluated_fact_ids_batch = AsyncMock(return_value={})
...
async def test_resolve_edges_handles_llm_error() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "other", [fact_id])])
    ctx.graph_engine.get_recent_edge_pairs = AsyncMock(return_value=set())
    ctx.graph_engine.get_evaluated_fact_ids_batch = AsyncMock(return_value={})
    ctx.graph_engine.record_negative_evaluations = AsyncMock(return_value=1)
...
async def test_resolve_edges_shared_fact_candidates() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "neural_networks", [fact_id])])
    ctx.graph_engine.get_recent_edge_pairs = AsyncMock(return_value=set())
    ctx.graph_engine.get_evaluated_fact_ids_batch = AsyncMock(return_value={})
...
async def test_resolve_edges_multiple_evidence_facts() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(other_id, "chloroplast", [fid1, fid2])])
    ctx.graph_engine.get_recent_edge_pairs = AsyncMock(return_value=set())
    ctx.graph_engine.get_evaluated_fact_ids_batch = AsyncMock(return_value={})
    ...
    ctx.graph_engine._fact_repo = AsyncMock()
    ctx.graph_engine._fact_repo.get_by_id = AsyncMock(side_effect=lambda fid: fact1 if fid == fid1 else fact2)
    ctx.graph_engine.get_facts_by_ids = AsyncMock(return_value=[fact1, fact2])

    # LLM returns a valid decision
    ctx.model_gateway.generate_json = AsyncMock(
        return_value={"results": [{"relationship_type": "related", "weight": 0.8, "justification": "test"}]}
    )
...
async def test_resolve_edges_staleness_filter() -> None:
    ...
    ctx.graph_engine.find_nodes_sharing_facts = AsyncMock(return_value=[(stale_id, "stale_node", [fact_id])])
...
async def test_resolve_edges_no_embedding_on_node() -> None:
    ...
    ctx.model_gateway.generate_json = AsyncMock(
        return_value={"results": [{"relationship_type": "related", "weight": 0.6, "justification": "test"}]}
    )
...
async def test_resolve_edges_multiple_candidates() -> None:
    ...
    ctx.graph_engine._fact_repo = AsyncMock()
    ctx.graph_engine._fact_repo.get_by_id = AsyncMock(side_effect=lambda fid: fact_a if fid == fid_a else fact_b)

...
async def reconnaissance(input: SuperSynthesizerInput, ctx: Context) -> dict[str, Any]:
    ...
    for term in search_terms:
        nodes = await read_engine.search_nodes(term, limit=30)
        for n in nodes:
            nid = str(n.id)
            if nid not in seen_ids:
                seen_ids.add(nid)
                facts = await read_engine.get_node_facts(n.id)
                all_nodes.append(
                    {
                        "node_id": nid,
                        "concept": n.concept,
    ...
    if input.existing_synthesis_ids:
        ...
        for sid in input.existing_synthesis_ids:
            try:
                node = await read_engine.get_node(_uuid.UUID(sid))
...
async def combine(input: SuperSynthesizerInput, ctx: Context) -> dict[str, Any]:
    ...
    try:
        ...
        await worker_engine.set_node_definition(supersynthesis_node_id, super_text)
        ...
        for sid in synthesis_node_ids:
            try:
                sub_node = await read_engine.get_node(uuid.UUID(sid))
        ...
        if write_session:
            ...
            await write_session.execute(
                update(WriteNode).where(WriteNode.node_uuid == supersynthesis_node_id).values(metadata_=existing_meta)
            )

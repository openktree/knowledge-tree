...
async def _ensure_qdrant_collections():
    """Ensure Qdrant collections exist before any tests run."""
    try:
        ...
        await QdrantFactRepository(client).ensure_collection()
        await QdrantNodeRepository(client).ensure_collection()
    except Exception:
        pass  # Qdrant may not be running in all test environments

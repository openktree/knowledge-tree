...
# Pool ground truth — loaded from proposals/pool_ground_truth.yaml if
# present. Generated by curate_pool.py from cached model outputs, edited
# manually for quality. Keys are lowercased names.
_GT_PATH = Path(__file__).resolve().parent / "proposals" / "pool_ground_truth.yaml"


def _load_pool_gt() -> dict[str, dict[str, dict]]:
    if not _GT_PATH.exists():
        return {}
    try:
        import yaml  # local import so datasets.py doesn't hard-require yaml
    except ImportError:
        return {}
    raw = yaml.safe_load(_GT_PATH.read_text(encoding="utf-8")) or {}
    return {str(k).strip().lower(): v for k, v in raw.items() if isinstance(v, dict)}


POOL_GROUND_TRUTH: dict[str, dict[str, dict]] = _load_pool_gt()

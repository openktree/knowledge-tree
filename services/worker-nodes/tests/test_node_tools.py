"""Unit tests for worker-nodes agent tools and pipelines.

Moved from services/worker-orchestrator/tests/test_agent_tools.py to enforce
the rule: no test in services/X/tests/ imports from services/Y/.
"""
...
from kt_worker_nodes.agents.tools.build_node import MAX_BATCH_SIZE as BUILD_MAX_BATCH_SIZE
from kt_worker_nodes.agents.tools.build_node import build_nodes_impl
from kt_worker_nodes.agents.tools.read_node import MAX_BATCH_SIZE as READ_MAX_BATCH_SIZE
from kt_worker_nodes.agents.tools.read_node import read_node_impl, read_nodes_impl
from kt_worker_nodes.pipelines.building.unified import UnifiedNodeBuilder
from kt_worker_nodes.pipelines.edges.classifier import EdgeClassifier, cap_facts_by_type
...
from kt_worker_nodes.pipelines.enrichment import PoolEnricher  # noqa: F401 — used in patches
...
_NOOP_ENRICH = EnrichResult(new_facts_linked=0, dimensions_regenerated=False)


@pytest.fixture(autouse=True)
def _mock_pool_enricher_in_read_node():
    """Auto-mock PoolEnricher.enrich in read_node so tests don't hit write-db."""
    with patch(
        "kt_worker_nodes.agents.tools.read_node.PoolEnricher",
        return_value=MagicMock(enrich=AsyncMock(return_value=_NOOP_ENRICH)),
    ):
        yield
...
async def test_read_nodes_caps_at_max_batch_size() -> None:
    ...
    ids = [uuid.uuid4() for _ in range(READ_MAX_BATCH_SIZE + 5)]
    nodes_by_id = {}
    for nid in ids:
        node = _make_mock_node(nid, f"concept_{nid}")
        node.node_type = "concept"
    ...
    assert result["count"] == READ_MAX_BATCH_SIZE
    assert result["capped"] is True


# ══════════════════════════════════════════════════════════════════════
# batch build_nodes tests
# ══════════════════════════════════════════════════════════════════════
...
async def test_build_nodes_caps_at_max_batch_size() -> None:
    ...
    entries = [{"name": f"concept_{i}", "node_type": "concept"} for i in range(BUILD_MAX_BATCH_SIZE + 5)]
    result = await build_nodes_impl(entries, ctx, state)

    assert result["count"] == BUILD_MAX_BATCH_SIZE
    assert result["capped"] is True
...
async def test_enrich_node_from_pool_returns_enrich_result() -> None:
    """PoolEnricher.enrich returns an EnrichResult — links seed facts, no dim regen."""
    ctx = _make_ctx()
    node = _make_mock_node(concept="enrich_test")

    new_fact = _make_mock_fact(content="new seed fact")
    fact_id = new_fact.id
    ...
    mock_seed = MagicMock()
    mock_seed.key = "seed:enrich_test"
    mock_seed.status = "promoted"

    ctx.graph_engine.get_facts_by_ids = AsyncMock(return_value=[new_fact])
    ctx.graph_engine.link_fact_to_node = AsyncMock()

    with (
        patch("kt_db.repositories.write_nodes.WriteNodeRepository") as MockWriteRepo,
        patch("kt_db.repositories.write_seeds.WriteSeedRepository") as MockSeedRepo,
    ):
        MockWriteRepo.return_value.get_by_uuid = AsyncMock(return_value=mock_wn)
        MockSeedRepo.return_value.get_seed_by_promoted_node_key = AsyncMock(return_value=mock_seed)
        MockSeedRepo.return_value.get_facts_for_seed = AsyncMock(return_value=[fact_id])
...
async def test_enrich_node_no_write_session_skips() -> None:
    """PoolEnricher.enrich returns zero when no write session is available."""
    ctx = _make_ctx()
    ctx.graph_engine._write_session = None
    node = _make_mock_node(concept="no_session")

    result = await PoolEnricher(ctx).enrich(node)

    assert isinstance(result, EnrichResult)
    assert result.new_facts_linked == 0
    assert result.dimensions_regenerated is False
...
async def test_build_node_enrich_does_not_regenerate_dimensions() -> None:
    ...
    with (
        ...
        patch(
            "kt_worker_nodes.pipelines.building.unified.PoolEnricher",
            return_value=MagicMock(enrich=AsyncMock(return_value=_NOOP_ENRICH)),
        ),
    ):
        result = await UnifiedNodeBuilder(ctx).build("enrich_regen_test", "concept", ctx, state)

    # With no-op enrichment (0 new facts), action is "read" not "enriched"
    assert result["action"] in ("read", "enriched")
...
def test_resolve_fact_tokens_replaces_indices() -> None:
    ...
    result = resolve_fact_tokens(raw, idx_map)
...
def test_resolve_fact_tokens_no_tokens() -> None:
    ...
    result = resolve_fact_tokens(raw, {1: uuid.uuid4()})
    assert result == raw
...
def test_cap_facts_by_type_preserves_diversity() -> None:
    ...
    result = cap_facts_by_type(all_facts, per_type_cap=20, total_cap=9)

...
from kt_facts.processing.seed_dedup import (
    _create_embedding_disambiguation,
    _llm_confirm_merge,
    _merge_pair,
    deduplicate_seed,
)
...
class TestDeduplicateSeed:
    async def test_no_matches_returns_same_key(self):
        ...
        result = await deduplicate_seed(
            "entity:albert-einstein", "Albert Einstein", "entity", repo,
    ...
    async def test_trigram_alone_does_not_merge(self):
        ...
        result = await deduplicate_seed(
            "entity:a-einstein", "A. Einstein", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
    ...
    async def test_alias_match_merges(self):
        repo = make_seed_repo_mock()
        existing = make_seed(
            "entity:karl-marx", "Karl Marx", "entity", fact_count=3,
        ...
        result = await deduplicate_seed(
            "entity:k-marx", "K. Marx", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
    ...
    async def test_merged_seed_skipped(self):
        ...
        result = await deduplicate_seed(
            "entity:new", "New Name", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
    ...
    async def test_embedding_dedup(self):
        ...
        result = await deduplicate_seed(
            "entity:einstein", "Einstein", "entity", repo,
            embedding_service=embedding_service,
            qdrant_seed_repo=qdrant_repo,
        )
        assert result == "entity:albert-einstein"
    ...
    async def test_trigram_high_embedding_low_no_merge(self):
        ...
        existing = make_seed(
            "entity:university-of-arizona", "The University of Arizona", "entity", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[existing])
    ...
    async def test_phonetic_with_embedding_floor_merges(self):
        ...
        existing = make_seed(
            "entity:democratic-party", "Democratic Party", "entity", fact_count=5,
        )
        repo.find_by_phonetic = AsyncMock(return_value=[existing])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
            "entity:democrtic-party": make_seed(
                "entity:democrtic-party", "Democrtic Party", "entity", fact_count=1,
            ),
        ...
        repo.find_similar_seeds = AsyncMock(side_effect=[
        ...
        with patch("kt_facts.processing.seed_dedup.get_settings") as mock_settings:
            ...
            result = await deduplicate_seed(
                "entity:democrtic-party", "Democrtic Party", "entity", repo,
                embedding_service=embedding_service,
                qdrant_seed_repo=qdrant_repo,
            )
    ...
    async def test_phonetic_with_embedding_below_floor_skips(self):
        ...
        existing = make_seed(
            "entity:bank-of-england", "Bank of England", "entity", fact_count=5,
        )
        repo.find_by_phonetic = AsyncMock(return_value=[existing])
        ...
        with patch("kt_facts.processing.seed_dedup.get_settings") as mock_settings:
            ...
            result = await deduplicate_seed(
                "entity:bank-of-america", "Bank of America", "entity", repo,
                embedding_service=embedding_service,
                qdrant_seed_repo=qdrant_repo,
            )
    ...
    async def test_invalid_name_skips_dedup(self):
        ...
        result = await deduplicate_seed(
            "entity:k-m-a", "K. M. A.", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
    ...
    async def test_et_al_name_skips_dedup(self):
        ...
        result = await deduplicate_seed(
            "entity:smith-et-al", "Smith et al.", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
...
class TestContainmentMismatch:
    ...
    def test_committee_vs_democrats_on_committee(self):
        assert _is_containment_mismatch(
    ...
    def test_symmetric(self):
        ...
        assert _is_containment_mismatch(
            "jeffrey epstein's lawyer", "jeffrey epstein"
        ) is True
...
class TestMultiMatchAmbiguity:
    ...
    async def test_multi_match_shared_ancestor_merges(self):
        ...
        ancestor = make_seed(
            "entity:ancestor", "Ancestor", "entity", status="active", fact_count=10,
        )
        merged_a = make_seed(
            "entity:cand-a", "Candidate A", "entity", status="merged", fact_count=3,
        )
        merged_a.merged_into_key = "entity:ancestor"
        merged_b = make_seed(
            "entity:cand-b", "Candidate B", "entity", status="merged", fact_count=2,
        )
        ...
        result = await deduplicate_seed(
            "entity:incoming", "Alias X", "entity", repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=make_qdrant_seed_repo_mock(),
        )
...
class TestContainmentGuardIntegration:
    async def test_trigram_match_with_containment_skipped(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "entity:jeffrey-epstein", "Jeffrey Epstein", "entity", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[existing])
...
class TestLLMConfirmMerge:
    async def test_llm_confirms_same_concept(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "concept:photosynthesis", "photosynthesis", "concept", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
            "concept:oxygenic-photosynthesis": make_seed(
                "concept:oxygenic-photosynthesis", "oxygenic photosynthesis", "concept", fact_count=1,
            ),
        ...
        result = await deduplicate_seed(
            "concept:oxygenic-photosynthesis", "oxygenic photosynthesis", "concept",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
    ...
    async def test_auto_merge_skips_llm_for_high_similarity(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "concept:posttraumatic-growth", "posttraumatic growth", "concept", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
            "concept:post-traumatic-growth": make_seed(
                "concept:post-traumatic-growth", "post-traumatic growth", "concept", fact_count=1,
            ),
        ...
        result = await deduplicate_seed(
            "concept:post-traumatic-growth", "post-traumatic growth", "concept",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
            model_gateway=model_gw,
    ...
    async def test_auto_merge_blocked_by_digit_guard(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "event:apvac1", "APVAC1", "event", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
    ...
    async def test_auto_merge_blocked_by_academic_initials(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "entity:ana-r-p-silva", "Ana R. P. Silva", "entity", fact_count=5,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
        ...
        await deduplicate_seed(
            "entity:ana-r-s-silva", "Ana R. S. Silva", "entity",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
            model_gateway=model_gw,
    ...
    async def test_llm_rejects_creates_anchor(self):
        ...
        repo = make_seed_repo_mock()
        existing = make_seed(
            "concept:light-dependent-reactions", "light-dependent reactions", "concept", fact_count=3,
        )
        incoming = make_seed(
            "concept:light-independent-reactions", "light-independent reactions", "concept", fact_count=1,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
        ...
        result = await deduplicate_seed(
            "concept:light-independent-reactions", "light-independent reactions", "concept",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
            model_gateway=model_gw,
    ...
    async def test_llm_unavailable_auto_merges(self):
        ...
        result = await deduplicate_seed(
            "concept:photo-new", "photosynthesis process", "concept",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
    ...
    async def test_no_facts_names_only(self):
        ...
        confirmed, preferred = await _llm_confirm_merge(
            "Calvin cycle", "concept:calvin-cycle",
            "Calvin-Benson cycle", "concept:calvin-benson-cycle",
            repo, write_fact_repo, model_gw,
        )
    ...
    async def test_anchor_skipped_on_recheck(self):
        ...
        repo = make_seed_repo_mock()
        ambiguous = make_seed(
            "concept:light-dependent-reactions", "light-dependent reactions",
            "concept", status="ambiguous", fact_count=3,
        )
        repo.find_similar_seeds = AsyncMock(return_value=[])
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
            "concept:new-light-concept": make_seed(
                "concept:new-light-concept", "new light concept", "concept", fact_count=1,
            ),
        ...
        result = await deduplicate_seed(
            "concept:new-light-concept", "new light concept", "concept",
            repo,
            embedding_service=make_embedding_service_mock(),
            qdrant_seed_repo=qdrant_repo,
        )
...
class TestPrefixDisambiguation:
    def test_light_dependent_vs_independent(self):
        assert _is_prefix_disambiguation_candidate(
...
class TestCascadingRouting:
    ...
    async def test_cascading_two_level_routing(self):
        ...
        repo.get_routes_for_parent = AsyncMock(side_effect=lambda k: {
            "concept:a": [route_a_b],
            "concept:b": [route_b_c],
        }.get(k, []))
        repo.get_seed_by_key = AsyncMock(side_effect=lambda k: {
    ...
    async def test_routing_stops_at_max_depth(self):
        ...
        from kt_facts.processing.seed_routing import _resolve_through_pipes, MAX_ROUTE_DEPTH
        ...
        result = await _resolve_through_pipes(
            "concept:s0", seeds["concept:s0"], "some fact", repo,
        )
    ...
    async def test_routing_stops_on_same_key(self):
        ...
        result = await _resolve_through_pipes(
            "concept:a", seed_a, "some fact", repo,
        )
...
class TestIsSafeAutoMerge:
    ...
    def test_high_score_synonyms_pass(self):
        assert is_safe_auto_merge(
            "post-traumatic growth", "posttraumatic growth", 0.987, 0.95,
        ) is True

    def test_high_score_digit_blocked(self):
        assert is_safe_auto_merge(
            "APVAC1", "APVAC2", 0.96, 0.95,
        ) is False

    def test_high_score_initials_blocked(self):
        assert is_safe_auto_merge(
            "Ana R. S. Silva", "Ana R. P. Silva", 0.98, 0.95,
        ) is False

    def test_below_threshold(self):
        assert is_safe_auto_merge(
            "post-traumatic growth", "posttraumatic growth", 0.93, 0.95,
        ) is False

    def test_containment_blocked(self):
        assert is_safe_auto_merge(
            "adult cancer patients", "adult cancer survivorship care", 0.96, 0.95,
        ) is False

    def test_low_string_similarity_blocked(self):
        assert is_safe_auto_merge(
            "alpha brainwave bands", "theta brain waves", 0.96, 0.95,
        ) is False
